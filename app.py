...
from models import db, User, Course, Class, Exercise, ExerciseAttempt, CourseFile, student_class_association, course_exercise
import json
import random
import string
...
def view_exercise(exercise_id):
    ...
    template = f'exercise_types/{exercise.exercise_type}_preview.html'
        ...
        try:
            ...
            if exercise_type == 'qcm':
                ...
                for index in sorted(question_indices):
                    ...
                    app.logger.debug(f'Question {index}:')
                    app.logger.debug(f'- Texte: {question_text}')
                    app.logger.debug(f'- Options: {options}')
                    app.logger.debug(f'- Index correct: {correct_index}')
                    ...
                    try:
                        ...
                        app.logger.error(f'Erreur de conversion pour l\'index correct: {correct_index}')
                        flash('Erreur lors de la création de l\'exercice.', 'error')
                        return redirect(url_for('create_exercise'))
                ...
                content = {'questions': questions}
                app.logger.debug(f'Contenu final de l\'exercice: {content}')
...
def submit_exercise(exercise_id):
    ...
    if exercise.exercise_type == 'qcm':
        ...
        for i in range(total_sentences):
            ...
            # Comparer avec les mots disponibles
            if answer and answer in content.get('words', []):
                correct_answers += 1
                feedback.append({
                    'sentence': i + 1,
                    'correct': True,
        ...
        score = (correct_answers / total_sentences) * 100 if total_sentences > 0 else 0
        ...
        # Récupérer les paires formées par l'utilisateur
        for i in range(total_pairs):
            left = request.form.get(f'left_{i}')
            right = request.form.get(f'right_{i}')
            if left and right:
                user_pairs[left] = right
                answers.append({'left': left, 'right': right})
        ...
        for pair in content['pairs']:
            ...
            if left in user_pairs and user_pairs[left] == right:
                correct_pairs += 1
                feedback.append({
                    'pair': {'left': left, 'right': right},
                    'correct': True,
                    'message': 'Paire correcte !'
                })
            else:
                feedback.append({
                    'pair': {'left': left, 'right': right},
                    'correct': False,
                    'message': 'Paire incorrecte'
                })
...
def submit_answer(exercise_id, course_id=0):
    ...
    if exercise.exercise_type == 'fill_in_blanks':
        ...
        total_blanks = len(content.get('answers', []))
        correct_answers = 0

        for i in range(total_blanks):
            student_answer = request.form.get(f'answer_{i}')
            print(f"[DEBUG] Réponse brute pour le trou {i}: {student_answer}")
            
            if not student_answer:  # Si pas de réponse
                print(f"[DEBUG] Pas de réponse pour le trou {i}")
                answers[f'answer_{i}'] = ''
                feedback.append({
                    'blank': i + 1,
                    'correct': False,
                    'feedback': f"Aucune réponse fournie."
                })
                continue

            # Nettoyer la réponse de l'étudiant
            student_answer = student_answer.lower().strip()
            answers[f'answer_{i}'] = student_answer
            
            # Vérifier si la réponse est correcte
            expected_answer = content['answers'][i]
            print(f"[DEBUG] Réponse attendue pour le trou {i}: {expected_answer}")
            
            if student_answer == expected_answer:
                print(f"[DEBUG] Trou {i} - Bonne réponse!")
                correct_answers += 1
                feedback.append({
                    'blank': i + 1,
                    'correct': True,
                ...
                print(f"[DEBUG] Trou {i} - Réponse incorrecte")
                feedback.append({
                    'blank': i + 1,
                    'correct': False,
                    'feedback': f"Incorrect. La bonne réponse était : {expected_answer}"
                })
        
        score = (correct_answers / total_blanks) * 100 if total_blanks > 0 else 0
...
@app.route('/submit_exercise_answer/<int:exercise_id>', methods=['POST'])
@login_required
def submit_exercise_answer(exercise_id):
    try:
        ...
        if exercise.exercise_type == 'qcm':
            ...
            answers = [request.form.get(f'answer_{i}', '') for i in range(len(content['sentences']))]
            correct_answers = [sentence['answer'] for sentence in content['sentences']]
            ...
            for i in range(len(content['draggable_items'])):
                if f'answer_{i}' in request.form:
                    user_order.append(int(request.form[f'answer_{i}']))
            correct_order = content['correct_order']
            score = sum(1 for a, b in zip(user_order, correct_order) if a == b)
            max_score = len(correct_order)

        else:
            flash('Type d\'exercice non pris en charge.', 'error')
            return redirect(url_for('view_exercise', exercise_id=exercise_id))
        ...
        flash(f'Exercice soumis ! Score : {score}/{max_score}', 'success')
        return redirect(url_for('view_exercise', exercise_id=exercise_id, course_id=course_id))
